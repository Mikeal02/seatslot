...
export const determineMovieStatus = (
...
export async function resolveTMDBMovieId(input: {
  ...
  const exactPoster = movies.find(
    (m) => m.poster_url && input.posterUrl && m.poster_url === input.posterUrl,
  );
  const exactBackdrop = movies.find(
    (m) =>
      m.backdrop_url &&
      input.backdropUrl &&
      m.backdrop_url === input.backdropUrl,
  );
  const exactDate = movies.find(
    (m) =>
      normalizeTitle(m.title) === wantedTitle && m.release_date === wantedDate,
  );
  const exactYear = movies.find(
    (m) =>
      normalizeTitle(m.title) === wantedTitle &&
      m.release_date?.slice(0, 4) === wantedYear,
  );
  const exactTitle = movies.find(
    (m) => normalizeTitle(m.title) === wantedTitle,
  );
...
export async function ensureMovieImported(
  ...
  options: {
    ...
    generateShowtimes?: boolean;
  } = {},
) {
  const movie =
    typeof tmdbMovieOrId === "number"
