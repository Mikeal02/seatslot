...
export default function MovieDetails() {
  ...
  const fetchTMDBRichDetails = useCallback(
    async (movieData: Movie & { tmdb_id?: number | null }) => {
      try {
          ...
          setMovie((prev) =>
            prev
              ? {
                  ...
                  tmdb_id: tmdbId,
                  duration_minutes:
                    details.duration_minutes || prev.duration_minutes,
                  genre: details.genre?.length ? details.genre : prev.genre,
                  cast_members: details.cast_members?.length
                    ? details.cast_members
                    : prev.cast_members,
                  director:
                    (typeof details.director === "string"
                      ? details.director
                      : details.director?.name) || prev.director,
                }
              : prev,
          );
        }
  ...
  const releaseState =
    releaseDate && releaseDate > new Date() ? "Upcoming" : "Released";
  ...
  const detailCards = [
    {
      label: "TMDB ID",
      value: tmdbDetails.tmdb_id || movie.tmdb_id || "Resolving",
      icon: Shield,
    },
    {
      label: "Release",
      value: releaseDate ? format(releaseDate, "MMM d, yyyy") : "TBA",
      icon: Calendar,
    },
    { label: "State", value: tmdbDetails.status || releaseState, icon: Tag },
    {
      label: "Votes",
      value: tmdbDetails.vote_count
        ? tmdbDetails.vote_count.toLocaleString()
        : "Fresh import",
      icon: Users,
    },
    {
      label: "Popularity",
      value: tmdbDetails.popularity
        ? Math.round(tmdbDetails.popularity).toLocaleString()
        : "Live sync",
      icon: TrendingUp,
    },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <section
          ref={heroRef}
          className="relative h-[58vh] min-h-[320px] sm:h-[65vh] sm:min-h-[520px] overflow-hidden"
        >
          <motion.div
          ...
          <div className="relative container mx-auto px-3 sm:px-6 h-full flex items-end pb-10">
            <div className="flex gap-6 md:gap-8 items-end w-full">
              ...
              >
                  ...
                  items={[
                    ...
                    ...(movie.genre?.[0]
                      ? [
                          {
                            label: movie.genre[0],
                            to: `/movies?genre=${encodeURIComponent(movie.genre[0])}`,
                          },
                        ]
                      : []),
                    { label: movie.title },
                ...
                <h1 className="text-3xl md:text-5xl lg:text-6xl font-black drop-shadow-lg tracking-tighter leading-[0.95]">
                  {movie.title}
                </h1>
                ...
                <div className="flex flex-wrap gap-2">
                  {movie.genre.map((g) => (
                    <Badge
                      key={g}
                      variant="secondary"
                      className="backdrop-blur-sm bg-secondary/80 font-medium"
                    >
                      {g}
                    </Badge>
                  ))}
                ...
                <div className="flex flex-wrap items-center gap-3 text-sm">
                  ...
                  {tmdbDetails.original_language && (
                    <div className="flex items-center gap-1.5 bg-background/30 backdrop-blur-sm rounded-full px-3 py-1.5 border border-border/10">
                      ...
                      <span>
                        {langMap[tmdbDetails.original_language] ||
        ...
        <section className="container mx-auto px-3 sm:px-6 py-6 sm:py-10">
          <div className="grid lg:grid-cols-3 gap-6 lg:gap-10 min-w-0">
                ...
                <TabsContent value="overview" className="space-y-8">
                  ...
                  {tmdbDetails.keywords && tmdbDetails.keywords.length > 0 && (
                    ...
                    >
                      {tmdbDetails.keywords.map((k) => (
                        <span
                          key={k}
                          className="text-[10px] px-2.5 py-1 rounded-full bg-muted/60 text-muted-foreground border border-border/20 font-medium hover:bg-muted hover:text-foreground transition-colors cursor-default"
                        >
                          {k}
                        </span>
                      ))}
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                      ...
                      {tmdbDetails.production_companies &&
                      ...
                      {tmdbDetails.production_countries &&
                        tmdbDetails.production_countries.length > 0 && (
                          <div className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border/30 hover:border-primary/20 transition-colors">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="font-semibold text-sm truncate">
                                {tmdbDetails.production_countries.join(", ")}
                              </p>
                            </div>
                          </div>
                        )}
                      {tmdbDetails.spoken_languages &&
                        tmdbDetails.spoken_languages.length > 0 && (
                          <div className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border/30 hover:border-primary/20 transition-colors">
                            <div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center shrink-0">
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="font-semibold text-sm truncate">
                                {tmdbDetails.spoken_languages.join(", ")}
                              </p>
                            </div>
                          </div>
                        )}
                      ...
                      {tmdbDetails.popularity && tmdbDetails.popularity > 0 && (
                        <div className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border/30 hover:border-primary/20 transition-colors">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="font-semibold text-sm">
                              {Math.round(tmdbDetails.popularity)}
                            </p>
                          </div>
                        </div>
                      )}
                      {tmdbDetails.homepage && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-3 p-4 rounded-xl bg-card border border-border/30 hover:border-primary/30 transition-colors group"
                        >
                          <div className="h-10 w-10 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="font-semibold text-sm text-primary group-hover:underline truncate">
                              Visit →
                            </p>
                          </div>
                        </a>
                  ...
                  <AnimatePresence>
                    {showtimes.length > 0 ? (
                      ...
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.4 }}
                        className="p-6 bg-muted/50 rounded-xl border border-border text-center"
                      >
                  ...
                  {!tmdbDetails.budget && !tmdbDetails.revenue && (
                    <div className="text-center py-12 text-muted-foreground">
                      ...
                      <p className="font-medium">
                        Box office data not available yet
                      </p>
                      <p className="text-sm mt-1">
                        Financial data will appear once the movie is released
                      </p>
                    </div>
                  )}
                </TabsContent>
            ...
            <div
              id="booking-panel"
              className="lg:col-span-1 order-first lg:order-last min-w-0 scroll-mt-32"
            >
              <motion.div
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-muted/50">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {movie.duration_minutes} min • {movie.genre[0]}
                        </p>
                      </div>
                    </div>
                        ...
                        >
                          <div className="text-sm space-y-2 p-3 rounded-xl bg-muted/30 border border-border/20">
                            <p>
                              ...
                              {format(
                                parseISO(selectedShowtime.show_date),
                                "EEEE, MMM d",
                              )}
                            </p>
                            <p>
                              ...
                              {format(
                                parseISO(
                                  `2000-01-01T${selectedShowtime.show_time}`,
                                ),
                                "h:mm a",
                              )}
                            </p>
                          ...
                          <Button
                            ...
                            className="w-full cinema-gradient btn-professional h-12 text-base font-bold rounded-xl"
                            size="lg"
                          >
                            Select Seats
                          </Button>
