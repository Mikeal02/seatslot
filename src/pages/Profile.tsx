...
export default function Profile() {
  ...
  const fetchProfileAndBookings = async () => {
    try {
      ...
      const { data: bookingsData, error: bookingsError } = await supabase
        .from("bookings")
        .select(
          `*, showtime:showtimes(*, movie:movies(*), screen:screens(*, theatre:theatres(*))), booked_seats:booked_seats(*, seat:seats(*))`,
        )
        .eq("user_id", user!.id)
  ...
  const totalTickets = confirmedBookings.reduce(
    (sum, b) => sum + (b.booked_seats?.length || 0),
    0,
  );
  const uniqueMovies = new Set(
    confirmedBookings.map((b) => b.showtime?.movie?.title),
  ).size;
            ...
            <div className="relative p-6 sm:p-10">
              <div className="flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="relative">
                  <div
                    className={`absolute -inset-1 rounded-full bg-gradient-to-br ${tier.color} opacity-60 blur-sm`}
                  />
                  ...
                  <div
                    className={`absolute -bottom-1 -right-1 h-8 w-8 rounded-full bg-gradient-to-br ${tier.color} flex items-center justify-center shadow-lg border-2 border-white/20`}
                  >
                ...
                <div className="flex-1 text-center sm:text-left">
                  ...
                  <div className="mt-3 flex items-center gap-3 justify-center sm:justify-start">
                    <Badge
                      className={`bg-gradient-to-r ${tier.color} text-white border-0 px-3 py-1 text-xs font-bold uppercase tracking-wider shadow-lg`}
                    >
              ...
              <div className="mt-6 max-w-sm mx-auto sm:mx-0">
                <div className="flex justify-between text-[10px] text-white/60 mb-1.5 uppercase tracking-wider font-medium">
                  ...
                  <span>
                    {tier.name === "Platinum"
                      ? "Max Tier"
                      : `Next: ${totalSpent >= 5000 ? "Platinum" : totalSpent >= 1000 ? "Gold" : "Silver"}`}
                  </span>
                </div>
              ...
              <Link key={action.label} to={action.href}>
                <Card className="border-border/20 hover:border-primary/30 transition-all duration-300 cursor-pointer group">
                  <CardContent className="p-3 sm:p-4 text-center">
                    <div
                      className={`h-10 w-10 rounded-xl ${action.color} flex items-center justify-center mx-auto mb-2 group-hover:scale-110 transition-transform`}
                    >
                ...
                <Card className="bg-card border-border/30">
                  <CardContent className="py-16 text-center">
                    ...
                    <p className="text-muted-foreground text-sm mb-6 max-w-sm mx-auto">
                      Start exploring movies and book your first ticket for an
                      unforgettable cinematic experience!
                    </p>
                    <Button
                      asChild
                      className="cinema-gradient btn-professional rounded-full px-6"
                    >
                              ...
                              <div className="flex-1 p-4 sm:p-5 flex flex-col justify-between min-w-0">
                                <div>
                                  <div className="flex items-start justify-between gap-3 mb-2">
                                    ...
                                    <Badge
                                      ...
                                      className={
                                        booking.booking_status === "confirmed"
                                          ? "cinema-gradient border-0 text-primary-foreground text-[10px] shrink-0"
                                          : "shrink-0 text-[10px]"
                                      }
                                    >
                                      {booking.booking_status === "confirmed"
                                  ...
                                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-muted-foreground">
                                    ...
                                    <div className="flex items-center gap-1.5">
                                      ...
                                      <span className="truncate">
                                        {booking.showtime?.screen?.theatre
                                          ?.name || "N/A"}
                                      </span>
                                    </div>
                                    <div className="flex items-center gap-1.5">
                                ...
                                <div className="flex items-center justify-between mt-3 pt-3 border-t border-border/30">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <span className="font-black text-base cinema-gradient-text">
                ...
                <Card className="bg-card border-border/30 glow-card">
                  ...
                  <CardContent className="space-y-5">
                    ...
                    <Button
                      onClick={handleSave}
                      disabled={saving}
                      className="cinema-gradient btn-professional rounded-xl px-6 w-full sm:w-auto"
                    >
