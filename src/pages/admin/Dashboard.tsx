...
export default function AdminDashboard() {
  ...
  const fetchAllData = async () => {
    try {
      const [moviesRes, bookingsRes, theatresRes, profilesRes, reviewsRes] =
        await Promise.all([
          supabase.from("movies").select("id, status, genre, rating"),
          supabase
            .from("bookings")
            .select(
              "id, total_amount, booking_status, created_at, showtime_id",
            ),
      ...
      setStats({
        ...
        nowShowingMovies: movies.filter((m) => m.status === "now_showing")
          .length,
  ...
  const statCards = [
    ...
    },
    {
      ...
      icon: Star,
      description: `From ${stats?.totalBookings || 0} reviews`,
