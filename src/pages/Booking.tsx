...
import {
  ConcessionSelector,
  SelectedConcession,
} from "@/components/booking/ConcessionSelector";
...
export default function Booking() {
  ...
  const { locks, bookedSeatIds: bookedSet } = useSeatLocks(
    showtimeId,
    user?.id,
  );
  const { reserve, release, markPaymentInitiated, earliestExpiry } =
  ...
  const timeLeft = earliestExpiry
    ? Math.max(0, Math.floor((earliestExpiry - now) / 1000))
    : 0;
  ...
  useEffect(() => {
    if (isExpired && selectedSeats.length > 0) {
      ...
      toast({
        ...
        description: "Your seat hold expired. Please select again.",
      });
    }
  }, [isExpired]);
  ...
  return (
    ...
    >
      ...
      <main
        id="main-content"
        className="flex-1 container mx-auto px-3 sm:px-4 py-4 sm:py-6 overflow-x-hidden"
      >
        {/* Movie Info Banner */}
        <motion.div
        ...
        >
          ...
          <div className="relative flex items-center gap-4 p-4 sm:p-5">
            <img
              ...
              alt={movie.title}
              className="w-16 h-24 rounded-lg object-cover shadow-lg border border-border/20 hidden sm:block"
            />
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <BackButton
                  fallback={`/movie/${movie.id}`}
              ...
              <h1 className="text-lg sm:text-xl font-black tracking-tight line-clamp-1">
                {movie.title}
              </h1>
              <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground mt-1">
                <span className="flex items-center gap-1">
                  <Calendar className="h-3 w-3" />
                  {format(parseISO(showtime.show_date), "EEE, MMM d")}
                </span>
                <span className="flex items-center gap-1">
        ...
        <div className="mb-6">
                ...
                <div key={step.id} className="flex items-center gap-2 flex-1">
                  <button
                    onClick={() => {
                      if (
                        isDone ||
                        (step.id === "extras" && selectedSeats.length > 0) ||
                        step.id === "seats"
                      ) {
                  ...
                  >
                    <motion.div
                      className={cn(
                        ...
                        isActive
                          ? "cinema-gradient border-transparent text-primary-foreground shadow-lg shadow-primary/25"
                          : isDone
          ...
          <Progress
            ...
            className="h-1 max-w-xl mx-auto rounded-full"
          />
        </div>
                ...
                >
                  ...
                  <div className="text-center relative">
                    ...
                    <p className="text-muted-foreground text-sm mt-1">
                      Confirm your details before secure payment
                    </p>
                  </div>
                  ...
                  <div className="space-y-4 relative">
                    <div className="p-4 rounded-xl bg-muted/20 border border-border/20 hover:border-border/30 transition-colors">
                      <div className="flex items-center gap-3 mb-3">
                        <img
                          src={movie.poster_url || "/placeholder.svg"}
                          alt=""
                          className="w-12 h-18 rounded-lg object-cover shadow-md"
                        />
                        <div>
                    ...
                    >
                      ...
                      <div className="relative flex justify-between items-center">
                        <div>
                          <p className="text-[10px] uppercase tracking-widest opacity-80 font-medium mb-0.5">
                            Total Amount
                          </p>
          ...
          <div className="flex items-center gap-3">
            {grandTotal > 0 && (
              <div className="text-sm">
                ...
                <p className="font-black text-base cinema-gradient-text">
                  ₹{grandTotal.toFixed(0)}
                </p>
              </div>
            )}
            <div className="flex-1">
              {currentStep === "confirm" ? (
                <Button
                  onClick={handleConfirmBooking}
                  disabled={booking || selectedSeats.length === 0}
                  className="w-full cinema-gradient btn-professional h-12 font-bold rounded-xl shadow-lg shadow-primary/20"
                  size="lg"
                >
                  {booking ? (
                    <motion.span
