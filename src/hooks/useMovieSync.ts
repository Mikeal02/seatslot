      ...
      slice.map(async (m) => {
        try {
          ...
          return {
            ...
            director:
              typeof d.director === "string"
                ? d.director
                : (d.director?.name ?? m.director),
            cast_members: d.cast_members?.length
              ? d.cast_members
              : m.cast_members,
...
export function useMovieSync() {
  ...
  const syncMovies = useCallback(
    async (forceSync = false): Promise<boolean> => {
      ...
      try {
        ...
        const latestReleasePage =
          (Math.floor(Date.now() / (1000 * 60 * 60 * 24 * 4)) % 5) + 1;
        ...
        const [latestReleaseRes, nowPlayingRes, upcomingRes] =
          await Promise.all([
            fetch(
        ...
        const [latestReleaseData, nowPlayingData, upcomingData] =
          await Promise.all([
        ...
        if (
          latestReleaseData.error ||
          nowPlayingData.error ||
          upcomingData.error
        ) {
          console.warn(
            "TMDB API error:",
        ...
        if (upcomingData.movies?.length > 0) {
          ...
          const futureMovies = upcomingData.movies.filter(
            (movie: TMDBMovie) => {
