...
export function BookingSummary({
  ...
  const seatsByType = selectedSeats.reduce(
    (acc, seat) => {
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">
              {movie.duration_minutes}m • {movie.genre?.slice(0, 2).join(", ")}
            </p>
            {movie.rating && (
        ...
        {selectedSeats.length > 0 && (
          <>
                    ...
                    <div className="flex flex-wrap gap-1 max-w-full">
                      {typeSeats.map((s) => (
                        <Badge
                          ...
                          variant="outline"
                          className={cn(
                            "max-w-full text-[10px] font-bold px-2 py-0.5",
                            typeColors[type],
                          )}
                        >
                          {s.row_label}
            ...
            <div className="space-y-2 text-xs">
              {Object.entries(seatsByType).map(([type, typeSeats]) => {
                const typeTotal = typeSeats.reduce(
        ...
        {selectedSeats.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-[10px] text-muted-foreground/60 mt-0.5">
              Tap on available seats in the layout
            </p>
          </div>
        )}
      </CardContent>
