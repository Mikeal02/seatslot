...
const categoryConfig: Record<
  string,
  { icon: React.ReactNode; emoji: string; gradient: string }
> = {
  snacks: {
...
export function ConcessionSelector({
  selectedItems,
  onItemsChange,
  ...
  const updateQuantity = (item: ConcessionItem, delta: number) => {
    ...
    if (newQty === 0) {
      ...
      if (existing) {
        onItemsChange(
          selectedItems.map((s) =>
            s.item.id === item.id ? { ...s, quantity: newQty } : s,
          ),
        );
      } else {
    ...
    <Card className="bg-card border-border/30 glow-card rounded-2xl overflow-hidden">
        ...
        <CardTitle className="flex items-center gap-2.5 text-base sm:text-lg">
          ...
          <div>
            ...
            <p className="text-[10px] sm:text-xs text-muted-foreground font-normal mt-0.5">
              Make your movie experience complete
            </p>
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent className="px-4 sm:px-6 pb-5">
        <Tabs defaultValue={categories[0] || "snacks"}>
          <TabsList
            className="w-full grid h-10 sm:h-11 bg-muted/30 rounded-xl p-1"
            ...
            {categories.map((cat) => {
              ...
              return (
                <TabsTrigger
                  key={cat}
                  value={cat}
                  className="capitalize text-[10px] sm:text-xs gap-1.5 rounded-lg data-[state=active]:shadow-md"
                >
                  {config?.icon}
                    ...
                    return (
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {isAdded && (
                              <motion.div
                                ...
                                animate={{ scale: 1 }}
                              >
                                <Badge className="bg-primary/20 text-primary text-[8px] px-1.5 py-0 border-0">
                                  Added
                                </Badge>
                              </motion.div>
                            )}
                          ...
                          <p className="text-sm sm:text-base font-black cinema-gradient-text mt-1">
                            ₹{item.price}
                          </p>
                        </div>
