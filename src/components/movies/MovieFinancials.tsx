      ...
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size} className="-rotate-90">
          ...
          <motion.circle
            ...
            animate={{
              strokeDashoffset: circumference - (circumference * capped) / 100,
            }}
...
export function MovieFinancials({
  budget,
  revenue,
  budgetFormatted,
  revenueFormatted,
  ...
  const calculatedRoi =
    roi ?? (budget && budget > 0 ? Math.round((profit / budget) * 100) : null);
  const multiplier =
    revenueMultiplier ??
    (budget && budget > 0
      ? Math.round(((revenue || 0) / budget) * 10) / 10
      : null);
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {budget && budget > 0 && (
          ...
          >
            ...
            <p className="text-2xl font-black tracking-tight">
              {budgetFormatted || `$${budget.toLocaleString()}`}
            </p>
        ...
        {revenue && revenue > 0 && (
          ...
          >
            ...
            <p className="text-2xl font-black tracking-tight cinema-gradient-text">
        ...
        {budget && budget > 0 && revenue && revenue > 0 && (
          ...
          >
            <div
              className={`absolute top-0 left-0 right-0 h-0.5 ${profitable ? "bg-accent" : "bg-destructive"}`}
            />
            <div className="flex items-center gap-2 text-muted-foreground text-xs mb-2">
              {profitable ? (
            ...
            <p
              className={`text-2xl font-black tracking-tight ${profitable ? "text-accent" : "text-destructive"}`}
            >
              {profitable ? "+" : "-"}
            ...
            {calculatedRoi !== null && (
              <p
                className={`text-xs mt-1 font-bold ${profitable ? "text-accent/80" : "text-destructive/80"}`}
              >
                {profitable ? "+" : ""}
                {calculatedRoi}% ROI
              </p>
      ...
      {budget && budget > 0 && revenue && revenue > 0 && (
        ...
        >
          <GaugeRing
            ...
            color={
              profitable ? "hsl(var(--accent))" : "hsl(var(--destructive))"
            }
          ...
          {calculatedRoi !== null && (
            <GaugeRing
              ...
              color={
                profitable ? "hsl(var(--primary))" : "hsl(var(--destructive))"
              }
          ...
          {multiplier !== null && (
            <div className="flex flex-col items-center gap-2">
              ...
              >
                <div
                  className={`w-16 h-16 rounded-full flex items-center justify-center border-2 ${profitable ? "border-accent/40 bg-accent/5" : "border-destructive/40 bg-destructive/5"}`}
                >
                  <span
                    className={`text-lg font-black ${profitable ? "text-accent" : "text-destructive"}`}
                  >
                    {multiplier}x
                  </span>
                </div>
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="font-bold cinema-gradient-text">
                {revenueFormatted}
              </span>
            </div>
            <div className="relative h-2.5 bg-muted rounded-full overflow-hidden">
