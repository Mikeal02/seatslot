...
export function SimilarMovies({
  movieTitle,
  tmdbId,
  similarMovies: propSimilar,
  recommendedMovies: propRecommended,
}: SimilarMoviesProps) {
  const [similar, setSimilar] = useState<TMDBSimilarMovie[]>(propSimilar || []);
  const [recommended, setRecommended] = useState<TMDBSimilarMovie[]>(
    propRecommended || [],
  );
  ...
  if (loading || (similar.length === 0 && recommended.length === 0))
    return null;
                    ...
                    <div className="absolute bottom-2 left-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="flex flex-wrap gap-1">
                        {m.genre.slice(0, 2).map((g) => (
                          <span
                            key={g}
                            className="text-[9px] bg-card/80 backdrop-blur-md px-1.5 py-0.5 rounded-full font-medium border border-border/20"
                          >
                            {g}
                          </span>
                        ))}
