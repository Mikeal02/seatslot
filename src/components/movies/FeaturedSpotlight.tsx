          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm mt-1.5">
                Handpicked top-rated films for you
              </p>
            </div>
          </div>
        </motion.div>
            ...
            <Link to={`/movie/${hero.id}`} className="group block">
              <div className="relative rounded-3xl overflow-hidden glow-card border border-border/20 aspect-[4/3]">
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-8 space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {hero.genre.slice(0, 3).map((g) => (
                      <Badge
                        key={g}
                        variant="secondary"
                        className="glass-card text-[10px] font-semibold uppercase tracking-wider"
                      >
                        {g}
                      </Badge>
                    ))}
                  ...
                  <Button
                    variant="cinema"
                    className="rounded-full px-6 h-11 text-sm font-bold mt-2 group/btn"
                  >
                      ...
                      <div>
                        <div className="flex gap-1.5 mb-2.5">
                          {movie.genre.slice(0, 2).map((g) => (
                            <span
                              key={g}
                              className="text-[9px] font-semibold bg-muted px-2 py-0.5 rounded-full uppercase tracking-wider"
                            >
                              {g}
                            </span>
                          ))}
                        ...
                        <p className="text-muted-foreground text-xs line-clamp-2 mt-2 leading-relaxed">
                          {movie.description}
                        </p>
                      </div>
