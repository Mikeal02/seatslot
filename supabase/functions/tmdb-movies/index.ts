...
function getCertification(
...
): { certification: string; country: string } | null {
  ...
  for (const country of preferred) {
    ...
    if (entry) {
      const cert = entry.release_dates.find(
  ...
  for (const entry of releaseDates.results) {
    ...
    if (cert)
      return { certification: cert.certification, country: entry.iso_3166_1 };
  }
  return null;
}
...
function transformMovie(movie: TMDBMovie, includeDetails = false) {
  ...
  // Keywords
  const keywords =
    movie.keywords?.keywords?.slice(0, 15).map((k) => k.name) || [];
  ...
  const recommendedMovies =
    movie.recommendations?.results?.slice(0, 10).map((m) => ({
      ...transformMovie(m),
      genre: m.genres?.map((g) => g.name) || [],
    })) || [];
  ...
  const collection = movie.belongs_to_collection
    ? {
        ...
        poster_url: movie.belongs_to_collection.poster_path
          ? `${TMDB_IMAGE_BASE}/w500${movie.belongs_to_collection.poster_path}`
          : null,
        backdrop_url: movie.belongs_to_collection.backdrop_path
          ? `${TMDB_IMAGE_BASE}/original${movie.belongs_to_collection.backdrop_path}`
          : null,
      }
    : null;
  ...
  const roi =
    movie.budget && movie.budget > 0 ? (profit / movie.budget) * 100 : null;
  const revenueMultiplier =
    movie.budget && movie.budget > 0
      ? (movie.revenue || 0) / movie.budget
      : null;
  ...
  return {
    ...
    director: director
      ? {
          ...
          photo: director.profile_path
            ? `${TMDB_IMAGE_BASE}/w185${director.profile_path}`
            : null,
        }
      : null,
    writers,
    ...
    profit: profit,
    profit_formatted:
      movie.revenue && movie.budget ? formatCurrency(Math.abs(profit)) : null,
    roi: roi !== null ? Math.round(roi) : null,
    revenue_multiplier:
      revenueMultiplier !== null
        ? Math.round(revenueMultiplier * 10) / 10
        : null,
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `TMDB request: action=${action}, movieId=${movieId}, query=${query}`,
    );
    ...
    switch (action) {
      case "now_playing": {
        const data = await fetchFromTMDB(
          `/movie/now_playing?page=${page}&region=US`,
        );
        result = {
      ...
      case "latest_releases": {
        ...
        result = {
          movies,
          total_pages: Math.min(data.total_pages || 1, 5),
          page: data.page,
        };
        break;
      }
      case "upcoming": {
        const data = await fetchFromTMDB(
          `/movie/upcoming?page=${page}&region=US`,
        );
        result = {
          movies: data.results.map((m: TMDBMovie) => transformMovie(m)),
          total_pages: data.total_pages,
      ...
      case "person": {
        ...
        const data = await fetchFromTMDB(
          `/person/${personId}?append_to_response=movie_credits,tv_credits,images,external_ids,tagged_images`,
        );
        ...
        const castFilmography = (data.movie_credits?.cast || [])
          ...
          .sort(
            (a: any, b: any) =>
              new Date(b.release_date || "1900").getTime() -
              new Date(a.release_date || "1900").getTime(),
          )
          .map((m: any) => ({
            tmdb_id: m.id,
            title: m.title,
        ...
        for (const c of crewFilmography) {
          if (!crewSeen.has(c.tmdb_id)) {
            ...
            const jobs = crewFilmography
              .filter((x: any) => x.tmdb_id === c.tmdb_id)
              .map((x: any) => x.job);
        ...
        const tvCredits = (data.tv_credits?.cast || [])
          ...
          .map((t: any) => ({
            ...
            poster_url: t.poster_path
              ? `${TMDB_IMAGE_BASE}/w300${t.poster_path}`
              : null,
        ...
        const avgRating =
          castFilmography.length > 0
            ? Math.round(
                (castFilmography.reduce(
                  (s: number, m: any) => s + m.rating,
                  0,
                ) /
                  castFilmography.length) *
                  10,
              ) / 10
            : 0;
        const highestRated =
          castFilmography.length > 0
            ? castFilmography.reduce(
                (best: any, m: any) => (m.rating > best.rating ? m : best),
                castFilmography[0],
              )
            : null;
        const decadeBreakdown: Record<string, number> = {};
        for (const m of castFilmography) {
          const decade = m.release_date
            ? `${Math.floor(parseInt(m.release_date.split("-")[0]) / 10) * 10}s`
            : "Unknown";
        ...
        result = {
          ...
          photo: data.profile_path
            ? `${TMDB_IMAGE_BASE}/h632${data.profile_path}`
            : null,
          ...
          // Photos
          photos:
            data.images?.profiles?.slice(0, 20).map((img: any) => ({
        ...
        const people = (data.results || [])
          ...
          .map((p: any) => ({
            ...
            photo: p.profile_path
              ? `${TMDB_IMAGE_BASE}/w200${p.profile_path}`
              : null,
