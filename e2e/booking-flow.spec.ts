...
test.describe("Booking flow", () => {
  test("select seats, pay, land on the confirmed ticket, and see it in history", async ({
    ...
    const seatButton = page.getByRole("button", {
      name: seatName(TARGET_SEAT),
    });
    await seatButton.click();
    await expect
      .poll(() => state.lockedByMe, {
        message: "seat should be locked server-side",
      })
    ...
    await page.getByRole("button", { name: /continue/i }).click();
    await expect(
      page.getByRole("heading", { name: /review & pay/i }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: `Pay ₹${TARGET_SEAT.price}` }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText(BOOKING_ID.slice(0, 8), { exact: false }).first(),
    ).toBeVisible();
  ...
  test("seats booked by someone else cannot be selected", async ({
    page,
    baseURL,
  }) => {
    const state = await installSupabaseMock(page, baseURL!);
    ...
    await expect(
      page.getByRole("button", { name: seatName(seats[1]) }),
    ).toBeDisabled();
    await expect(
      page.getByRole("button", { name: seatName(seats[2]) }),
    ).toBeDisabled();
    await expect(
      page.getByRole("button", { name: seatName(seats[3]) }),
    ).toBeEnabled();
  });

  test("booking history is gated behind authentication", async ({
    page,
    baseURL,
  }) => {
