...
function fakeJwt() {
  ...
  return [
    ...
    b64url({ sub: USER_ID, email: TEST_EMAIL, role: 'authenticated', exp, aud: 'authenticated' }),
    'e2e-signature',
  ].join('.');
}
...
export async function installSupabaseMock(page: Page, baseURL: string): Promise<MockState> {
  ...
  await page.route('**/functions/v1/**', async (route) => {
    ...
    if (path.endsWith('create-booking-payment')) {
      ...
      return json(route, { url: `${baseURL}/payment-success?session_id=cs_test_e2e_session` });
    }

    if (path.endsWith('verify-booking-payment')) {
      state.bookedSeatIds = Array.from(new Set([...state.bookedSeatIds, ...state.lockedByMe]));
    ...
    if (resource.startsWith('rpc/')) {
      ...
      if (fn === 'acquire_seat_locks') {
        ...
        const rows = ids.map((seat_id) => {
          const taken =
            state.bookedSeatIds.includes(seat_id) || state.lockedByOthers.includes(seat_id);
          if (!taken) state.lockedByMe = Array.from(new Set([...state.lockedByMe, seat_id]));
          return {
            seat_id,
            success: !taken,
            reason: taken
